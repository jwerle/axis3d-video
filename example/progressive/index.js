...
// at the very least, a ProgressiveVideoTexture is needed for rendering
// a video to a texture intelligently. VideoMaterial is a convenient wrapper
// around a fragment shader that usesUV data from the mesh to draw texture
...
const texture = new ProgressiveVideoTexture(ctx)
...
domElement.onerror = (e) => {
  console.error("error for '%s':", e.detail || e)
}
...
function scene() {
  ...
  camera({position: [0, 0, 1]}, () => {
    ...
    // enough data to give to the texture
    texture({domElement}, ({textureResolution, textureData}) => {
