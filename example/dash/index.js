...
// at the very least, a DASHVideoTexture is needed for rendering
...
const texture = new DASHVideoTexture(ctx)
...
// bescause it will throw an error as .m3u8 (dash) is not supported
// natively on most browsers
const src = '/assets/dash/big-buck-bunny.mpd'
...
function scene({time}) {
  ...
  camera({position: [0, 0, 0.5]}, () => {
    ...
    // enough data to give to the texture. the internal DASH
    // object will attach the DOM element as media and
    // load the source given source. All options in the
    // `.dash` object are passed directly to the constructor
    texture({domElement, src, dash: {debug: true}},
